...
periodically boradcasting updates to anyone who happens to be listening 
on port 2066. The child acts as the server, receiving the broadcasts and 
displaying the data.
*/
...
/* -----------------------------------------------------------------------*/
	...
	/* Enter our service loop, receiving packets and displaying
		them in some appropriate way.
	*/
	while (1) {
		/* Receive an update packet */
		client_len = sizeof client;
		recvfrom(sock, (char *)&pkt, sizeof pkt, 0, (struct sockaddr *)&client, &client_len);
...
/* This version displays p.text, p.x, and p.y in a table indexed by the text name */
...
#define TSIZE 50
	...
	int i, found;
/* Search the table for an entry with a matching text string. if found, 
the ordinal position of the entry in the table is used to determine the 
position on the screen at which this entry will be displayed. The x, y
values are only displayed; they are NOT kept in the table. If no matching
